...
@FunctionalInterface
public interface CheckedToDoubleBiFunction<T, U, EX extends Exception> {

	double applyOrThrow(T t, U u) throws EX;
	 ...
	 */
	default ToDoubleBiFunction<T, U> sneakyThrow() {
		return (t, u) -> {
			@SuppressWarnings("unchecked")
			CheckedToDoubleBiFunction<T, U, RuntimeException> sneaky = (CheckedToDoubleBiFunction<T, U, RuntimeException>) this;
			return sneaky.applyOrThrow(t, u);
		};
	}	
	
	@FunctionalInterface
	public interface Adapter<T, U, EX extends Exception> extends CheckedToDoubleBiFunction<T, U, EX>, ToDoubleBiFunction<T, U> {
		default double applyOrThrow(T t, U u) throws EX {
	...
	public interface Helper<T, U, EX extends Exception> extends CheckedToDoubleBiFunction<T, U, EX> {
	
		/**
		 * In case an exception occurs, an empty Optional is returned
		 */
		default BiFunction<T, U, OptionalDouble> optional() {
			return (t, u) -> {
				try {
					return OptionalDouble.of(applyOrThrow(t, u));
				} catch (Exception e) {
		 ...
		 */
		default CheckedToDoubleBiFunction.Adapter<T, U, EX> fallbackTo(ToDoubleBiFunction<? super T, ? super U> other) {
		 ...
		 * result.
		 */
		default CheckedToDoubleBiFunction.Adapter<T, U, EX> orReturn(double value) {
			return (t, u) -> {
				try {
					return applyOrThrow(t, u);
				} catch (Exception e) {
		 ...
		 */
		default CheckedToDoubleBiFunction.Adapter<T, U, EX> rethrowUnchecked(Function<? super EX, ? extends RuntimeException> mapper) {
		 ...
		 */
		default <EX2 extends Exception> CheckedToDoubleBiFunction.Helper<T, U, EX2> rethrow(Function<? super EX, EX2> mapper) {
	        Objects.requireNonNull(mapper);
		 ...
		 */
		default CheckedToDoubleBiFunction.Helper<T, U, EX> except(Consumer<? super EX> exceptionHandler) {	
		 ...
		 */
		default <EX2 extends EX> CheckedToDoubleBiFunction.Helper<T, U, EX> except(
		 ...
		 */
		default CheckedToDoubleBiFunction.Helper<T, U, EX> exceptUnchecked(
				Consumer<? super RuntimeException> exceptionHandler) {
			return exceptUnchecked(RuntimeException.class, exceptionHandler);
		}
		
		default <REX extends RuntimeException> CheckedToDoubleBiFunction.Helper<T, U, EX> exceptUnchecked(
		 ...
		 * exception handler. Afterwards, the exception is rethrown.
		 */
		default CheckedToDoubleBiFunction.Helper<T, U, EX> orTry(CheckedToDoubleBiFunction<? super T, ? super U, ? extends EX> other) {
	        Objects.requireNonNull(other);
