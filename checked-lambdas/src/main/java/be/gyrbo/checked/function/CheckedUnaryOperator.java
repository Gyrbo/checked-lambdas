...
@FunctionalInterface
public interface CheckedUnaryOperator<T, EX extends Exception> extends CheckedFunction<T, T, EX> {
	 ...
	 */
	default UnaryOperator<T> sneakyThrow() {
		return t -> {
			@SuppressWarnings("unchecked")
			CheckedUnaryOperator<T, RuntimeException> sneaky = (CheckedUnaryOperator<T, RuntimeException>) this;
			return sneaky.applyOrThrow(t);
		};
	}	
	
	@FunctionalInterface
	public interface Adapter<T, EX extends Exception> extends CheckedUnaryOperator<T, EX>, CheckedFunction.Adapter<T, T, EX>, UnaryOperator<T> {
		default T applyOrThrow(T t) throws EX {
	...
	public interface Helper<T, EX extends Exception> extends CheckedUnaryOperator<T, EX> {
	
		/**
		 * In case an exception occurs, an empty Optional is returned
		 */
		default Function<T, Optional<T>> optional() {
			return t -> {
				try {
		 ...
		 */
		default CheckedUnaryOperator.Adapter<T, EX> fallbackTo(UnaryOperator<T> other) {
		 ...
		 * result.
		 */
		default CheckedUnaryOperator.Adapter<T, EX> orReturn(T value) {
			return t -> {
				try {
					return applyOrThrow(t);
				} catch (Exception e) {
		 ...
		 */
		default <EX2 extends Exception> CheckedUnaryOperator.Helper<T, EX2> rethrow(Function<? super EX, EX2> mapper) {
	        Objects.requireNonNull(mapper);
		 ...
		 */
		default CheckedUnaryOperator.Helper<T, EX> except(Consumer<? super EX> exceptionHandler) {	
		 ...
		 */
		default <EX2 extends EX> CheckedUnaryOperator.Helper<T, EX> except(
		 ...
		 */
		default CheckedUnaryOperator.Helper<T, EX> exceptUnchecked(
				Consumer<? super RuntimeException> exceptionHandler) {
			return exceptUnchecked(RuntimeException.class, exceptionHandler);
		}
		
		default <REX extends RuntimeException> CheckedUnaryOperator.Helper<T, EX> exceptUnchecked(
		 ...
		 * exception handler. Afterwards, the exception is rethrown.
		 */
		default CheckedUnaryOperator.Helper<T, EX> orTry(CheckedUnaryOperator<T, ? extends EX> other) {
	        Objects.requireNonNull(other);
