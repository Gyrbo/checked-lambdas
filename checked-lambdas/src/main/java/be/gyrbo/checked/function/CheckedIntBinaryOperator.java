...
@FunctionalInterface
public interface CheckedIntBinaryOperator<EX extends Exception> {
	int applyOrThrow(int t, int u) throws EX;
	 ...
	 */
	default IntBinaryOperator sneakyThrow() {
		return (t, u) -> {
			@SuppressWarnings("unchecked")
			CheckedIntBinaryOperator<RuntimeException> sneaky = (CheckedIntBinaryOperator<RuntimeException>) this;
			return sneaky.applyOrThrow(t, u);
		};
	}	
	
	@FunctionalInterface
	public interface Adapter<EX extends Exception> extends CheckedIntBinaryOperator<EX>, IntBinaryOperator {
		default int applyOrThrow(int t, int u) throws EX {
	...
	public interface Helper<EX extends Exception> extends CheckedIntBinaryOperator<EX> {
		 ...
		 */
		default CheckedIntBinaryOperator.Adapter<EX> fallbackTo(IntBinaryOperator other) {
		 ...
		 * result.
		 */
		default CheckedIntBinaryOperator.Adapter<EX> orReturn(int value) {
			return (t, u) -> {
				try {
					return applyOrThrow(t, u);
				} catch (Exception e) {
		 ...
		 */
		default CheckedIntBinaryOperator.Adapter<EX> rethrowUnchecked(Function<? super EX, ? extends RuntimeException> mapper) {
		 ...
		 */
		default <EX2 extends Exception> CheckedIntBinaryOperator.Helper<EX2> rethrow(Function<? super EX, EX2> mapper) {
	        Objects.requireNonNull(mapper);
		 ...
		 */
		default CheckedIntBinaryOperator.Helper<EX> except(Consumer<? super EX> exceptionHandler) {	
		 ...
		 */
		default <EX2 extends EX> CheckedIntBinaryOperator.Helper<EX> except(
		 ...
		 */
		default CheckedIntBinaryOperator.Helper<EX> exceptUnchecked(
				Consumer<? super RuntimeException> exceptionHandler) {
			return exceptUnchecked(RuntimeException.class, exceptionHandler);
		}
		
		default <REX extends RuntimeException> CheckedIntBinaryOperator.Helper<EX> exceptUnchecked(
		 ...
		 * exception handler. Afterwards, the exception is rethrown.
		 */
		default CheckedIntBinaryOperator.Helper<EX> orTry(CheckedIntBinaryOperator<? extends EX> other) {
	        Objects.requireNonNull(other);
